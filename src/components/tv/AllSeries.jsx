  ...
  return (
    <div className='contain'>
      ...
      <div className="movies-pagination mt-6">
        <div onClick={()=>setPageNo(p=>Math.max(p-1,1))} className='page-box'>Prev</div>
        <div className='page-box'>{pageNo} / {totalPages}</div>
        <div onClick={()=>setPageNo(p=>Math.min(p+1,totalPages))} className='page-box'>Next</div>
      </div>
    </div>
  )
}
